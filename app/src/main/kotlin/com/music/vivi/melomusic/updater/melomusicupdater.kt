...
sealed class MeloUpdateStatus {
    object Idle : MeloUpdateStatus()
    object Checking : MeloUpdateStatus()
    ...
    data class NoUpdate(val version: String) : MeloUpdateStatus()
    data class Error(val message: String) : MeloUpdateStatus()
}
...
fun UpdateScreen(navController: NavHostController) {
    ...
    var status by remember { mutableStateOf<MeloUpdateStatus>(MeloUpdateStatus.NoUpdate(BuildConfig.VERSION_NAME)) }
    ...
    fun triggerUpdateCheck() {
        status = MeloUpdateStatus.Checking
        scope.launch {
            ...
            checkForUpdate(
                ...
                onSuccess = { tag, isAvailable, changelog, size, date, description, imageUrl, apkUrl ->
                    ...
                    status = if (isAvailable) {
                        ...
                        MeloUpdateStatus.NoUpdate(tag)
                    }
                },
                onError = {
                    status = MeloUpdateStatus.Error(context.getString(R.string.cant_check_updates))
                }
            )
        }
    }
    ...
    Scaffold(
        topBar = {
            LargeTopAppBar(
                title = {
                    val titleText = if (status is MeloUpdateStatus.Available) {
                        buildAnnotatedString {
                            ...
                            withStyle(
                                ...
                                append((status as MeloUpdateStatus.Available).version)
                            }
                        }
                    } else {
            ...
            Surface(
                ...
                Column(
                    ...
                    when (val currentStatus = status) {
                        is MeloUpdateStatus.Idle, is MeloUpdateStatus.Checking, is MeloUpdateStatus.NoUpdate, is MeloUpdateStatus.Error -> {
                            AnimatedActionButton(
                                text = stringResource(R.string.check_for_update),
                                onClick = { triggerUpdateCheck() },
                                enabled = currentStatus !is MeloUpdateStatus.Checking && !isDownloading,
                                modifier = Modifier.fillMaxWidth()
                            )
                        }

                        is MeloUpdateStatus.Available -> {
                            Row(
                                modifier = Modifier.fillMaxWidth(),
                                ...
                                AnimatedActionButton(
                                    ...
                                    onClick = {
                                        if (isDownloadComplete) {
                                            ...
                                            val urlToDownload = currentStatus.apkUrl ?: "https://github.com/MeloMusicApp/Melo-Music/releases/download/${currentStatus.version}/melomusic.apk"
        ...
        Box(
            ...
            LazyColumn(
                ...
                item {
                    val contentModifier = if (status is MeloUpdateStatus.Available) {
                    ...
                    Box(
                        ...
                        when (val currentStatus = status) {
                            is MeloUpdateStatus.Checking -> {
                                androidx.compose.material3.ContainedLoadingIndicator(
                                    modifier = Modifier.size(64.dp)
                                )
                            }

                            is MeloUpdateStatus.NoUpdate -> {
                                Column(
                                    horizontalAlignment = Alignment.CenterHorizontally
                                ) {
                                    Icon(
...
suspend fun checkForUpdate(
    ...
    withContext(Dispatchers.IO) {
        try {
            val url = URL("https://api.github.com/repos/MeloMusicApp/Melo-Music/releases/latest")
            ...
            if (shouldShow) {
                ...
                try {
                    val changelogUrl =
                        URL("https://github.com/MeloMusicApp/Melo-Music/releases/download/$tagWithPrefix/changelog.json")
